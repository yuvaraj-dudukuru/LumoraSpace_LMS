...
Flow Nexus swarms integrate seamlessly with Codex Flow hooks:

```bash
# Pre-task coordination setup
npx Codex-flow@alpha hooks pre-task --description "Initialize swarm"

# Post-task metrics export
npx Codex-flow@alpha hooks post-task --task-id "swarm-execution"
```
...
Codex mcp add flow-nexus npx flow-nexus@latest mcp start
```
...
**Remember**: Flow Nexus provides cloud-based orchestration infrastructure. For local execution and coordination, use the core `Codex-flow` MCP server alongside Flow Nexus for maximum flexibility.
