---
name: "source-command-swarm-examples"
description: "Migrated source command `swarm-examples`"
---

# source-command-swarm-examples

Use this skill when the user asks to run the migrated source command `swarm-examples`.
...
#### Using CLI (Fallback)
```bash
npx Codex-flow swarm "research AI trends" \
  --strategy research \
  --mode distributed \
  --max-agents 6 \
  --parallel
...
#### Using CLI (Fallback)
```bash
npx Codex-flow swarm "analyze codebase" \
  --strategy analysis \
  --mode mesh \
  --parallel \
  --timeout 300
