...
description: "Migrated source command `hooks-post-edit`"
---

# source-command-hooks-post-edit

Use this skill when the user asks to run the migrated source command `hooks-post-edit`.
...
```bash
npx Codex-flow hook post-edit -f "api/auth.js" --memory-key "auth/login-implementation"
```

### Format and validate

```bash
npx Codex-flow hook post-edit -f "config/webpack.js" --auto-format --validate-output
```

### Neural training

```bash
npx Codex-flow hook post-edit -f "utils/helpers.ts" --train-patterns --memory-key "utils/refactor"
```
...
This hook is automatically called by Codex when:
...
```bash
# After editing files
npx Codex-flow hook post-edit --file "path/to/edited.js" --memory-key "feature/step1"
```
