...
description: "Migrated source command `automation-self-healing`"
---

# source-command-automation-self-healing

Use this skill when the user asks to run the migrated source command `automation-self-healing`.
...
```json
{
  "PostToolUse": [{
    "matcher": "^Bash$",
    "command": "npx Codex-flow hook post-bash --exit-code '${tool.result.exitCode}' --auto-recover"
  }]
}
```
