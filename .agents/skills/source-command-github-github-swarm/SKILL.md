...
description: "Migrated source command `github-github-swarm`"
---

# source-command-github-github-swarm

Use this skill when the user asks to run the migrated source command `github-github-swarm`.
...
```bash
npx Codex-flow github swarm --repository owner/repo
```

### Maintenance-focused swarm

```bash
npx Codex-flow github swarm -r owner/repo -f maintenance --issue-labels
```

### Development swarm with PR automation

```bash
npx Codex-flow github swarm -r owner/repo -f development --auto-pr --code-review
```

### Full-featured triage swarm

```bash
npx Codex-flow github swarm -r owner/repo -a 8 -f triage --issue-labels --auto-pr
```
