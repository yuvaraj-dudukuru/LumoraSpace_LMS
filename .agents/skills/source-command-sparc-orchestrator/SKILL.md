...
description: "Migrated source command `sparc-orchestrator`"
---

# source-command-sparc-orchestrator

Use this skill when the user asks to run the migrated source command `sparc-orchestrator`.
...
```bash
# Initialize orchestration swarm
npx Codex-flow swarm init --topology hierarchical --strategy auto --max-agents 8

# Spawn coordinator agent
npx Codex-flow agent spawn --type coordinator --capabilities "task-planning,resource-management"

# Orchestrate tasks
npx Codex-flow task orchestrate --task "feature development" --strategy parallel --deps "auth,ui,api"
```
...
```bash
# 1. Initialize orchestration swarm
npx Codex-flow swarm init --topology hierarchical --max-agents 10

# 2. Create workflow
npx Codex-flow workflow create --name "feature-development" --steps "design,implement,test,deploy"

# 3. Execute orchestration
npx Codex-flow sparc run orchestrator "develop user management system" --parallel --monitor
