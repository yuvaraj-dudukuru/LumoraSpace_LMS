...
description: "Migrated source command `analysis-COMMAND_COMPLIANCE_REPORT`"
---

# source-command-analysis-command-compliance-report

Use this skill when the user asks to run the migrated source command `analysis-COMMAND_COMPLIANCE_REPORT`.
...
## Overview
Reviewed all command files in `.Codex/commands/analysis/` directory to ensure proper usage of:
- `mcp__claude-flow__*` tools (preferred)
- `npx Codex-flow` commands (as fallback)
...
The analysis directory is now fully compliant with the Codex Flow command standards.
