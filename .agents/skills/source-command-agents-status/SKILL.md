---
name: "source-command-agents-status"
description: "Show detailed status of an agent"
---

# source-command-agents-status

Use this skill when the user asks to run the migrated source command `agents-status`.
...
```bash
npx Codex-flow agent status <agent-id>
npx Codex-flow agent status --id <agent-id>
```
...
```bash
# Get status by ID
npx Codex-flow agent status coder-lx7m9k2

# Using --id flag
npx Codex-flow agent status --id researcher-abc123

# JSON output
npx Codex-flow agent status coder-lx7m9k2 --format json
```
...
```json
{
  ...
  "config": {
    "provider": "anthropic",
    "model": "Codex-3-5-sonnet-20241022",
    "timeout": 300,
    "autoTools": true
  },
  "metrics": {
...
- `npx Codex-flow agent metrics` - Aggregate metrics
- `npx Codex-flow agent health` - Health check
- `npx Codex-flow agent logs <id>` - Activity logs
