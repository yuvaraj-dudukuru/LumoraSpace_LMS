...
description: "Migrated source command `github-sync-coordinator`"
---

# source-command-github-sync-coordinator

Use this skill when the user asks to run the migrated source command `github-sync-coordinator`.
...
## Purpose
Multi-package synchronization and version alignment with ruv-swarm coordination for seamless integration between Codex-flow and ruv-swarm packages.
...
Bash(`gh api repos/:owner/:repo/contents/Codex-flow/Codex-flow/package.json \
  --method PUT \
  ...
  -f sha="$(gh api repos/:owner/:repo/contents/Codex-flow/Codex-flow/package.json?ref=sync/package-alignment --jq '.sha')")`)
...
```javascript
// Synchronize AGENTS.md files across packages using gh CLI
// Get file contents
CLAUDE_CONTENT=$(Bash("gh api repos/:owner/:repo/contents/ruv-swarm/docs/AGENTS.md --jq '.content' | base64 -d"))

// Update Codex-flow AGENTS.md to match using gh CLI
...
// Update file
Bash(`gh api repos/:owner/:repo/contents/Codex-flow/Codex-flow/AGENTS.md \
  --method PUT \
  -f message="docs: Synchronize AGENTS.md with ruv-swarm integration patterns" \
  -f branch="sync/documentation" \
  -f content="$(echo '# Codex Configuration for ruv-swarm\n\n[synchronized content]' | base64)" \
  -f sha="$(gh api repos/:owner/:repo/contents/Codex-flow/Codex-flow/AGENTS.md?ref=sync/documentation --jq '.sha' 2>/dev/null || echo '')")`)

// Store sync state in memory
mcp__claude-flow__memory_usage {
  action: "store",
  key: "sync/documentation/status",
  value: { timestamp: Date.now(), status: "synchronized", files: ["AGENTS.md"] }
}
```
...
mcp__github__push_files {
  ...
  files: [
    {
      path: "Codex-flow/Codex-flow/.Codex/commands/github/github-modes.md",
      content: "[GitHub modes documentation]"
    },
    {
      path: "Codex-flow/Codex-flow/.Codex/commands/github/pr-manager.md", 
      content: "[PR manager documentation]"
    },
    {
      path: "ruv-swarm/npm/src/github-coordinator/Codex-hooks.js",
...
### Integration Points
- Codex-flow: GitHub command modes in .Codex/commands/github/
- ruv-swarm: GitHub coordination hooks and utilities
- Documentation: Synchronized AGENTS.md instructions
...
[Single Message - Complete Synchronization]:
  ...
  Read("/workspaces/ruv-FANN/Codex-flow/Codex-flow/package.json")
  Read("/workspaces/ruv-FANN/ruv-swarm/npm/package.json")
  Read("/workspaces/ruv-FANN/Codex-flow/Codex-flow/AGENTS.md")
  Read("/workspaces/ruv-FANN/ruv-swarm/docs/AGENTS.md")
  
  // Synchronize multiple files simultaneously
  mcp__github__push_files {
    branch: "sync/complete-integration",
    files: [
      { path: "Codex-flow/Codex-flow/package.json", content: "[aligned package.json]" },
      { path: "Codex-flow/Codex-flow/AGENTS.md", content: "[synchronized AGENTS.md]" },
      { path: "Codex-flow/Codex-flow/.Codex/commands/github/github-modes.md", content: "[GitHub modes]" }
    ],
    message: "feat: Complete package synchronization with GitHub integration"
  }
  
  // Run validation tests
  Bash("cd /workspaces/ruv-FANN/Codex-flow/Codex-flow && npm install")
  Bash("cd /workspaces/ruv-FANN/Codex-flow/Codex-flow && npm test")
...
const docSyncPattern = {
  sourceOfTruth: "ruv-swarm/docs/AGENTS.md",
  targets: [
    "Codex-flow/Codex-flow/AGENTS.md",
    "AGENTS.md"  // Root level
  ],
  customSections: {
    "Codex-flow": "GitHub Commands Integration",
