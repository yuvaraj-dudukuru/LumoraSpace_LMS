---
name: "source-command-agents-pool"
description: "Manage agent pool for scaling"
---

# source-command-agents-pool

Use this skill when the user asks to run the migrated source command `agents-pool`.
...
```bash
npx Codex-flow agent pool [options]
```
...
npx Codex-flow agent pool

# Set pool size
npx Codex-flow agent pool --size 5

# Configure auto-scaling
npx Codex-flow agent pool --min 2 --max 15 --auto-scale

# Pre-warm agents for fast response
npx Codex-flow agent pool --warmup

# Disable auto-scaling
npx Codex-flow agent pool --auto-scale false
```
...
Configure in `.Codex-flow/config.yaml`:
...
- `npx Codex-flow agent spawn` - Manual agent spawning
- `npx Codex-flow agent list` - View active agents
- `npx Codex-flow swarm scale` - Swarm-level scaling
