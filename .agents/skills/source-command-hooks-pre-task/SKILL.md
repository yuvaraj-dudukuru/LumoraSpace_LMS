...
description: "Migrated source command `hooks-pre-task`"
---

# source-command-hooks-pre-task

Use this skill when the user asks to run the migrated source command `hooks-pre-task`.
...
```bash
npx Codex-flow hook pre-task -d "Continue API development" --load-memory
```

### Manual agent control

```bash
npx Codex-flow hook pre-task -d "Debug issue #123" --auto-spawn-agents false
```

### Full optimization

```bash
npx Codex-flow hook pre-task -d "Refactor codebase" --optimize-topology --estimate-complexity
```
...
npx Codex-flow hook pre-task --description "Your task here"
```
