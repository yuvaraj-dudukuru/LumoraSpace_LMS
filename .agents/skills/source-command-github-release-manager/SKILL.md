...
description: "Migrated source command `github-release-manager`"
---

# source-command-github-release-manager

Use this skill when the user asks to run the migrated source command `github-release-manager`.
...
mcp__github__push_files {
  ...
  files: [
    {
      path: "Codex-flow/Codex-flow/package.json",
...
Bash("cd /workspaces/ruv-FANN/Codex-flow/Codex-flow && npm run test")
Bash("cd /workspaces/ruv-FANN/Codex-flow/Codex-flow && npm run lint")
Bash("cd /workspaces/ruv-FANN/Codex-flow/Codex-flow && npm run build")
...
[Single Message - Complete Release Management]:
  ...
  Write("/tmp/release-v1.0.72/Codex-flow/Codex-flow/package.json", "[updated package.json]")
  ...
  Bash("cd /workspaces/ruv-FANN/Codex-flow/Codex-flow && npm install && npm test && npm run lint && npm run build")
  ...
  release-validation:
    ...
    steps:
      ...
      - name: Install and Test
        run: |
          cd Codex-flow/Codex-flow && npm install && npm test
          cd ../../ruv-swarm/npm && npm install && npm test:all
      - name: Validate Release
        run: npx Codex-flow release validate
```
