---
name: "source-command-automation-auto-agent"
description: "Migrated source command `automation-auto-agent`"
---

# source-command-automation-auto-agent

Use this skill when the user asks to run the migrated source command `automation-auto-agent`.
...
```bash
npx Codex-flow auto agent [options]
```
...
```bash
npx Codex-flow auto agent --task "Build a REST API with authentication"
```

### Constrained spawning

```bash
npx Codex-flow auto agent -t "Debug performance issue" --max-agents 3
```

### Analysis only

```bash
npx Codex-flow auto agent -t "Refactor codebase" --no-spawn
```

### Minimal strategy

```bash
npx Codex-flow auto agent -t "Fix bug in login" -s minimal
```
