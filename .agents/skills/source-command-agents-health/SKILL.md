---
name: "source-command-agents-health"
description: "Show agent health and metrics"
---

# source-command-agents-health

Use this skill when the user asks to run the migrated source command `agents-health`.
...
```bash
npx Codex-flow agent health [agent-id] [options]
```
...
npx Codex-flow agent health

# Specific agent health
npx Codex-flow agent health coder-lx7m9k2

# Continuous monitoring
npx Codex-flow agent health --watch

# Custom interval
npx Codex-flow agent health -w -i 10

# JSON output
npx Codex-flow agent health --format json
```
...
- `npx Codex-flow agent status` - Detailed agent info
- `npx Codex-flow agent metrics` - Performance metrics
- `npx Codex-flow doctor` - System-wide health
