...
Models marked `// PHASE 2` are kept in the schema (and the initial migration — migrations are cheap) but get no seed data, no API endpoints, and no UI in the 18-screen MVP (`DECISIONS.md` Q23).

```prisma
// ── Identity & Auth ──────────────────────────────────────────────
// Auth.js v5 owns Account/Session/VerificationToken via its Prisma
// adapter (standard tables below, not derived from any screen).
// NOTE: Auth.js's own Session model collides in name with our
// live-class session concept — ours is named LiveSession throughout
// this schema (see "Live Sessions" section) to avoid the clash.
// User itself is extended with the fields every screen reads.
...
model User {
  ...
  hostedSessions     LiveSession[]     @relation("SessionHost") // PHASE 2
...
model Enrollment {
  ...
  @@unique([userId, batchId]) // a learner cannot join the same batch twice
}
...
// PHASE 2
// Named LiveSession, not Session — Auth.js's Prisma adapter owns the
// literal `Session` model (see the Identity & Auth section above).
model LiveSession {
  id          String            @id @default(cuid())
  batchId     String
  batch       Batch             @relation(fields: [batchId], references: [id], onDelete: Cascade)
