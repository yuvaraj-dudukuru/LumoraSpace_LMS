...
- `updateEnrollmentStatusSchema`, `createUserSchema`, `updateUserRoleSchema`, `setUserStatusSchema`, `assignMentorSchema`, `revokeCertificateSchema` — Zod, mirror the existing per-domain validation files.
...
### `src/lib/certificates.ts` — M5c
- `issueCertificateIfEligible(enrollmentId): Promise<Certificate | null>` — idempotent. Re-fetches the enrollment fresh (never trusts a caller-supplied percent); `null` unless `progressPercent === 100`; `null` if a `VALID` certificate already exists for `(userId, programId)` — re-issuing after a `REVOKED` one is allowed, only a second `VALID` one is refused. Otherwise sets `Enrollment.status = COMPLETED` + `completedAt`, generates the next `LUM-{year}-{00001..}` number, and creates the certificate — retrying the create once on a `P2002` sequence-number collision.
...
| `/mentor/submissions/[submissionId]` | `getSubmissionForReview` → `requireMentorForBatch(submission.batchId)` — no separate `requireRole` needed, `requireMentorForBatch` already calls `requireUser` and rejects non-mentors/non-admins | `getSubmissionForReview`; `claimForReview`/`saveReviewDraft`/`submitReview` Server Actions, each independently re-resolving the batch and re-authorizing (see Rules) |
| `/admin` (layout) | `requireRole(ADMIN)` | — |
| `/admin` (page) | `requireRole(ADMIN)` | `getAdminDashboardStats` |
| `/admin/enrollments` | `requireRole(ADMIN)` | `getEnrollmentsForAdmin` (accessState/status/search query params, default `accessState=AWAITING`); `grantAccess`/`suspendAccess`/`updateEnrollmentStatus`/`bulkGrantAccess` Server Actions, each re-fetching the enrollment server-side before checking its current state |
| `/admin/users` | `requireRole(ADMIN)` | `getUsersForAdmin` (role/search query params); `createUser` Server Action |
| `/admin/users/[userId]` | `requireRole(ADMIN)`, then `forbidden()` when `getUserDetailForAdmin` returns `null` | `getUserDetailForAdmin`, `getAllBatchesForAdmin` (only when `role === MENTOR`); `updateUserRole`/`setUserStatus`/`assignMentorToBatch`/`removeMentorFromBatch` Server Actions |
...
- **"Last active" is derived, not tracked (M5b).** `User.lastActiveAt` exists in the schema but is never written anywhere in this app (confirmed by grep) — always `null`. Mentor-facing "last active" is `MAX(LessonProgress.completedAt, Submission.submittedAt, Attempt.startedAt)` per enrollment, same "derived not tracked" posture as Learning Hours. A learner with zero activity signals shows as `null`/"no activity yet", which counts as maximally stale for the at-risk rule below, not as an error.
...
- **Bulk-granting enrollment access is one `updateMany`, not a loop (M5c).** `bulkGrantAccess` filters `accessState: "AWAITING"` inside the query itself, so a stale selection (a row already granted by someone else between page load and click) is silently skipped rather than erroring — safe to click without re-verifying every row first.
...
- **Certificate issuance is derived from `progressPercent === 100`, through the single `refreshEnrollmentProgress` call site (M5c).** `issueCertificateIfEligible` is called there and nowhere else — both `markLessonComplete` and `submitAttempt`'s quiz-completion path already funnel through that rollup (M4.5), so both gain certificate issuance for free. The percent check is exact, never fuzzy: `getProgramProgress` rounds to `Math.round(x*1000)/10`, so full completion is always precisely `100`.
- **Certificate numbers are `LUM-{year}-{5-digit sequence}`, generated by finding the current max for that year and incrementing (M5c).** The only real race is two learners finishing in the same second computing the same next number — `issueCertificateIfEligible` retries the `create` once on a `P2002` conflict, recomputing the max before the retry. Re-issuing after a certificate is `REVOKED` is intentionally allowed; only a second `VALID` certificate for the same `(userId, programId)` is refused.
