...
| `COMMUNITY_URL` | No — optional | `src/app/learn/layout.tsx` — read server-side on every learner page render, passed to the shell as a plain string | **No nav item.** When set to an absolute `http(s)` URL (e.g. your Discord invite or forum), the learner sidebar gains a "Community" item that opens it in a new tab (`rel="noopener noreferrer"`). Unset or not an absolute `http(s)` URL = the item is simply absent; nothing else changes. |
